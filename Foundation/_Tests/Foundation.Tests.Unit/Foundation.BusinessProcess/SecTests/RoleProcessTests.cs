...
namespace Foundation.Tests.Unit.Foundation.BusinessProcess.SecTests
{
    ...
    public class RoleProcessTests : CommonBusinessProcessTests<IRole, IRoleProcess, IRoleRepository>
    {
        ...
        {
            IRoleRepository retVal = Substitute.For<IRoleRepository>();
        ...
        {
            IRoleProcess process = new RoleProcess(CoreInstance, RunTimeEnvironmentSettings, dateTimeService, LoggingService, TheRepository!, StatusRepository!, UserProfileRepository!,  ReportGenerator!);

            return process;
        }

        protected override IRole CreateBlankEntity(Int32 entityId)
        {
        ...
        {
            IRole retVal = CreateBlankEntity(entityId);
