//-----------------------------------------------------------------------
// <copyright file="NationalRegionViewModel.cs" company="JDV Software Ltd">
//     Copyright (c) JDV Software Ltd. All rights reserved.
// </copyright>
//-----------------------------------------------------------------------

using Foundation.Common;
using Foundation.Interfaces;

namespace Foundation.ViewModels.Core
{
    /// <summary>
    /// The User Interface interaction logic for National Region maintenance
    /// </summary>
    /// <seealso cref="GenericDataGridViewModel{INationalRegion}" />
    [DependencyInjectionTransient]
    public class NationalRegionViewModel : GenericDataGridViewModel<INationalRegion>, INationalRegionViewModel
    {
        /// <summary>
        /// Initialises a new instance of the <see cref="NationalRegionViewModel"/> class.
        /// </summary>
        /// <param name="core">The Foundation Core service.</param>
        /// <param name="runTimeEnvironmentSettings">The runtime environment settings.</param>
        /// <param name="dateTimeService">The date time service.</param>
        /// <param name="wpfApplicationObjects">The wpf application objects collection.</param>
        /// <param name="fileApi">The file service.</param>
        /// <param name="nationalRegionProcess">The national region process.</param>
        public NationalRegionViewModel
        (
            ICore core,
            IRunTimeEnvironmentSettings runTimeEnvironmentSettings,
            IDateTimeService dateTimeService,
            IWpfApplicationObjects wpfApplicationObjects,
            IFileApi fileApi,
            INationalRegionProcess nationalRegionProcess
        ) :
            base
            (
                core,
                runTimeEnvironmentSettings,
                dateTimeService,
                wpfApplicationObjects,
                fileApi,
                nationalRegionProcess
            )
        {
            LoggingHelpers.TraceCallEnter(core, runTimeEnvironmentSettings, dateTimeService, wpfApplicationObjects, fileApi, nationalRegionProcess);

            LoggingHelpers.TraceCallReturn();
        }
    }
}
